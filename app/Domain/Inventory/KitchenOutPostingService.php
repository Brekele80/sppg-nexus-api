...
class KitchenOutPostingService
{
    ...
    public function post(string $kitchenOutId, Request $request): array
    {
        ...
        return DB::transaction(function () use ($kitchenOutId, $request, $u, $companyId, $idempotencyKey) {
            ...
            // 2) Company boundary via branch
            $branch = DB::table('branches')
                ->where('id', (string) $ko->branch_id)
                ->select(['id', 'company_id'])
                ->first();

            if (!$branch || (string) $branch->company_id !== (string) $companyId) {
            ...
            if (empty($allowed) || !in_array((string) $ko->branch_id, $allowed, true)) {
            ...
            foreach ($lines as $idx => $ln) {
                ...
                if ($inventoryItemId === '') {
                    throw ValidationException::withMessages([
                        "lines.$lineIdx.inventory_item_id" => ['inventory_item_id is required'],
                    ]);
                }
                ...
                // Lock inventory item row (branch guard)
                ...
                foreach ($lots as $lot) {
                    ...
                    if (bccomp($lotRemaining, $consume, 3) < 0) {
                        throw new HttpException(
                            409,
                            "FIFO violation: lot {$lot->id} has {$lotRemaining}, attempted {$consume}"
                        );
                    }
                    ...
                    // Insert movement OUT with signed negative qty
                    ...
                    $consumption[] = [
                        ...
                        'unit'                => $invItem->unit !== null ? (string) $invItem->unit : null,
                        'inventory_lot_id'    => (string) $lot->id,
                        'lot_code'            => (string) ($lot->lot_code ?? ''),
                        'consumed_qty'        => $consume,
                        'movement_id'         => $moveId,
            ...
            // 6) Recompute on_hand from lots (truth) inside same TX
