...
use App\Http\Controllers\DC\SupplierMappingController;
use App\Http\Controllers\DC\AutoProcurementController;
use App\Http\Controllers\DC\MenuForecastController;
...
Route::middleware(['supabase', 'requireCompany'])->group(function () {
    ...
    */
    Route::prefix('dc')->group(function () {
        ...
        Route::get('/menus/{id}', [MenuController::class, 'show'])->whereUuid('id');

        // Supplier mappings (Phase 5.5)
        Route::get('/supplier-maps', [SupplierMappingController::class, 'index']);
        Route::get('/menus/{menu}/supplier-mappings', [SupplierMappingController::class, 'byMenu'])
            ->whereUuid('menu');

        // Forecast preview (Phase 5.5 UX support)
        Route::post('/menus/{menu}/forecast', [MenuForecastController::class, 'preview'])
            ->whereUuid('menu');
    });
    ...
    Route::middleware(['idempotency'])->group(function () {
        ...
        */
        Route::middleware(['requireRole:DC_ADMIN,CHEF'])->prefix('dc')->group(function () {
            ...
            Route::post('/menus/{id}/publish', [MenuController::class, 'publish'])->whereUuid('id');

            // Supplier Mapping
            Route::post('/supplier-maps', [SupplierMappingController::class, 'store']);
            Route::post('/menus/{menu}/supplier-mappings', [SupplierMappingController::class, 'storeForMenu'])
                ->whereUuid('menu');

            // Auto PR
            Route::post('/menus/{menu}/auto-pr', [AutoProcurementController::class, 'generatePR'])
                ->whereUuid('menu');
        });
        ...
        */
        Route::middleware(['requireRole:DC_ADMIN'])->prefix('dc')->group(function () {
            ...
            Route::post('/inventory/movements/{id}/void', [InventoryMovementVoidController::class, 'void'])
                ->whereUuid('id');
